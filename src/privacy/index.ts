...
- **AI and agent requests (handled by your provider, not us).** When you use AI or agent features, your request goes directly to the AI provider you have configured — it is not sent to or collected by NevoFlux. See "AI and Agent Features" below.
- **Shared Micro Apps (encrypted).** If you choose to share a Micro App, an encrypted archive of it is uploaded to and stored on nevoflux.app. The encryption password is generated on your device and is never uploaded, so we cannot read its contents. See "Micro App Sharing" below.
...
you** — they do not run in the background without your action — and they use an AI model
provider that **you configure** in NevoFlux (for example, your own provider account or API
key, or a local / self-hosted model).

When you use one of these features, the content required to fulfill your request (for
example, the text of a page you ask about or an instruction you type) is sent **directly
from your device to the AI provider you configured**. NevoFlux does not route this content
through, store it on, or receive it on our own servers (including nevoflux.app). Your
chosen provider processes the request under its own terms and privacy policy, so how your
data is handled — including whether it may be used to improve or train models — is governed
by that provider, not by us; we encourage you to review your provider's privacy policy. If
you configure a local or self-hosted model, this processing stays entirely on your own
device or infrastructure.

## Micro App Sharing

NevoFlux lets you share a Micro App with others. Sharing is **optional and initiated by
you** — nothing is uploaded unless you choose to share.

When you share a Micro App, NevoFlux packages it into a compressed archive and **encrypts
that archive on your device**. Only the **encrypted** archive is uploaded to nevoflux.app,
where it is stored so it can be retrieved through a share link. The password that encrypts
and decrypts the archive is **generated locally in your browser and is never uploaded to
nevoflux.app** — it is carried in the share link, or shared with the recipient however you
choose. Because we never receive this password, NevoFlux cannot decrypt or read the
contents of a shared Micro App.

## Third-Party Services

NevoFlux relies on a small number of third parties to function, which may include the AI
model provider you configure (when you use AI or agent features); software update and distribution services
...
for the purposes described in this policy, or as required by law. Encrypted Micro App
archives you choose to share are stored on nevoflux.app to provide the sharing feature;
because they are encrypted with a password we never receive, we cannot access their
contents.
...
- **AI 与智能体请求（由您的供应商处理，而非我们）。** 当您使用 AI 或智能体功能时，您的请求会直接发送至您所配置的 AI 供应商，不会发送给 NevoFlux，我们也不会收集这些内容。详见下文"AI 与智能体功能"。
- **分享的 Micro App（已加密）。** 如果您选择分享 Micro App，其加密后的压缩文件会被上传并保存在 nevoflux.app 上。加密密码在您的设备上生成、绝不上传，因此我们无法读取其内容。详见下文"Micro App 分享"。
...
不会在后台运行——并且使用**您在 NevoFlux 中自行配置**的 AI 模型供应商（例如您自己的供应商
账户或 API 密钥，或本地 / 自托管模型）。

当您使用这些功能时，完成请求所需的内容（例如您询问的页面文本，或您输入的指令）会**从您的
设备直接发送至您所配置的 AI 供应商**。NevoFlux 不会通过我们自己的服务器（包括 nevoflux.app）
转发、存储或接收这些内容。您所选择的供应商将依据其自身的条款和隐私政策处理请求，因此您的
数据如何被处理——包括是否可能被用于改进或训练模型——由该供应商决定，而非由我们决定；我们
建议您查阅所用供应商的隐私政策。如果您配置的是本地或自托管模型，则此类处理将完全保留在您
自己的设备或基础设施上。

## Micro App 分享

NevoFlux 允许您与他人分享 Micro App。分享是**可选的、由您主动发起的**——除非您选择分享，
否则不会上传任何内容。

当您分享 Micro App 时，NevoFlux 会将其打包为压缩文件，并**在您的设备上对该压缩文件进行
加密**。只有**加密后的**压缩文件会被上传至 nevoflux.app 并保存，以便通过分享链接进行获取。
用于加密和解密该压缩文件的密码**在您的浏览器本地生成，绝不会上传至 nevoflux.app**——它包含
在分享链接中，或由您自行以其他方式传递给接收方。由于我们从不接收该密码，NevoFlux 无法解密
或读取所分享 Micro App 的内容。

## 第三方服务

NevoFlux 依赖少量第三方来实现其功能，可能包括：您所配置的 AI 模型供应商（当您使用 AI 或智能体功能时）；
...
要求的期限内予以保留。您选择分享的加密 Micro App 压缩文件会保存在 nevoflux.app 上以提供分享
功能；由于它们使用我们从不接收的密码加密，我们无法访问其内容。
